...
const fetchSunsetData = async () => {
  const result = await fetch(
    `http://api.sunrise-sunset.org/json?formatted=0&lat=${latitude}&lng=${longitude}`
  );
...
export const handleSunsetRequest = async (ctx: Context) => {
  try {
    ...
    ctx.response.headers.append('access-control-allow-origin', 'http://localhost:1234');
